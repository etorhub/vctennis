...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  const user = session?.user
    ? {
        ...
        apartmentFloor: (session.user as { apartmentFloor?: string | null }).apartmentFloor ?? null,
        apartmentDoor: (session.user as { apartmentDoor?: number | null }).apartmentDoor ?? null,
