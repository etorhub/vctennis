...
- Rate limiting (issue #56): Better Auth's `rateLimit` is DB-backed (`storage: "database"`, `RateLimit` table in [`db/config.ts`](db/config.ts)) so limits survive Netlify Functions cold starts. Custom rules on `/sign-in/email`, `/sign-up/email`, `/forget-password` (5 requests/window, per IP) in [`src/lib/auth.ts`](src/lib/auth.ts); `scripts/verify-rate-limit.js` (in `npm run ci`) guards the config
